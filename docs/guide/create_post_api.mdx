...
In this guide, you will learn how to create a POST API.

We will use create a simple API that create an user.

To create a new POST API, you need to create a `handler placeholder (Dish)` and a `handler (Cooker)`.
...
type CreateUserRequest struct {
	FirstName string `required:"true"` // Required will set the field as required in the swagger definition
...
type UserApi struct {
  ...
  // Group of User related definitions
    ...
    // Third is the response parameter
		CreateUser kitchen.Dish[*SimpleDependency, *model.CreateUserRequest, *model.CreateUserResponse]
	}
}
...
type IUserManagement interface {
	CreateUser(ctx kitchen.IContext[*recipe.SimpleDependency], req *model.CreateUserRequest) (*model.CreateUserResponse, error)
}
...
With this setup, you have created a new POST API at `http://localhost:<YOUR_PORT>/v1/user`
